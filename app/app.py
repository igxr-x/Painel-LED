...
def resource_path(name):
    """Caminho de um recurso, funcionando no .py e dentro do .exe (PyInstaller)."""
    base = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
    return os.path.join(base, name)


# Identidade visual
BRAND = "TOPTRUCKS MOTORSPORTS"
BRAND_RED = "#c8102e"
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Painel LED Lambda - Configurador  |  " + BRAND)
    ...
    def _build_header(self):
        """Faixa superior com a logo TOPTRUCKS MOTORSPORTS."""
        header = tk.Frame(self, bg="white")
        header.pack(fill="x", side="top")

        self._logo_img = self._load_logo(target_h=58)
        if self._logo_img is not None:
            tk.Label(header, image=self._logo_img, bg="white").pack(
                side="left", padx=14, pady=8)
        else:
            tk.Label(header, text=BRAND, bg="white", fg=BRAND_RED,
                     font=("Segoe UI", 20, "bold")).pack(side="left", padx=14, pady=12)

        tk.Label(header, text="Painel LED Lambda", bg="white", fg="#333",
                 font=("Segoe UI", 11)).pack(side="right", padx=16)
        # linha vermelha de destaque abaixo da faixa
        tk.Frame(self, bg=BRAND_RED, height=3).pack(fill="x", side="top")

    def _load_logo(self, target_h=58):
        """Carrega a logo ja no tamanho do cabecalho (PNG pre-renderizado)."""
        try:
            return tk.PhotoImage(file=resource_path("LOGO_header.png"))
        except Exception:
            pass
        # fallback: usa a logo grande com subsample inteiro
        try:
    ...
    def _build_cfg_tab(self):
        ...
        ttk.Label(g2, text="Intensidade do alerta:").grid(row=2, column=0, sticky="e", padx=4, pady=4)
        self.alert_bright = tk.IntVar(value=255)
        ttk.Scale(g2, from_=1, to=255, variable=self.alert_bright,
                  orient="horizontal", length=220).grid(row=2, column=1, columnspan=3, sticky="w", padx=4)
        ttk.Label(g2, textvariable=self.alert_bright).grid(row=2, column=4, sticky="w")
        ttk.Label(g2, text="(brilho SO do alerta, independente do brilho global)",
                  foreground="#666").grid(row=3, column=0, columnspan=5, sticky="w", padx=4)
        ...
        # Indicador central quando o valor cai ABAIXO de um limite (extremo oposto
        # do LED de repouso): mostra so o quadrado 2x2 central, com cor propria.
        self.low_enable = tk.IntVar(value=0)
        ttk.Checkbutton(g7, variable=self.low_enable,
                        text="Indicador central quando ABAIXO do lambda "
                             "(quadrado 2x2 central, substitui a barra)").grid(
            row=2, column=0, columnspan=3, sticky="w", padx=6, pady=4)
        ttk.Label(g7, text="Cor abaixo:").grid(row=2, column=3, sticky="e", padx=4)
        self.low_color = "#0000ff"
        self.low_btn = tk.Button(g7, text="   ", width=4, bg=self.low_color,
                                 command=self.pick_low_color)
        self.low_btn.grid(row=2, column=4, padx=4)
        ttk.Label(g7, text="Lambda limite (abaixo disso, acende o indicador):").grid(
            row=3, column=0, columnspan=3, sticky="w", padx=6, pady=4)
        self.low_lambda = tk.StringVar(value="0.80")
        ttk.Entry(g7, textvariable=self.low_lambda, width=8).grid(row=3, column=3, padx=4)
    ...
    def _build_gps_tab(self):
        ...
        ttk.Label(g1, text="Tempo entre bolinhas (s):").grid(row=1, column=0, sticky="e", padx=4, pady=4)
        self.bolim_debounce_s = tk.StringVar(value="2.0")
        ttk.Entry(g1, textvariable=self.bolim_debounce_s, width=8).grid(row=1, column=1, padx=4)
        ttk.Label(g1, text="(regra: apos contar uma bolinha, espera esse tempo ate contar outra)",
        ...
        ttk.Button(g3, text="Salvar ponto nesta pista",
                   command=self.save_point_to_track).grid(row=0, column=4, padx=4)

        ttk.Label(g3, text="Ponto (lat, lon):").grid(row=1, column=0, sticky="e", padx=4, pady=4)
        self.point_lat = tk.StringVar(value=""); self.point_lon = tk.StringVar(value="")
        ttk.Entry(g3, textvariable=self.point_lat, width=13).grid(row=1, column=1, padx=2)
        ttk.Entry(g3, textvariable=self.point_lon, width=13).grid(row=1, column=2, padx=2)
        ttk.Button(g3, text="Capturar ponto aqui",
                   command=self.capture_point).grid(row=1, column=3, padx=4)

        ttk.Label(g3, text="Raio de deteccao (m):").grid(row=2, column=0, sticky="e", padx=4, pady=6)
        self.range_m = tk.StringVar(value="30")
        ttk.Entry(g3, textvariable=self.range_m, width=6).grid(row=2, column=1, sticky="w", padx=2)
        ttk.Label(g3, text="(pode capturar com o caminhao PARADO no box; funciona parado)",
                  foreground="#666").grid(row=2, column=2, columnspan=3, sticky="w", padx=4)
        ...
        self.g_lbl_dist = tk.StringVar(value="--")
        ...
        ttk.Label(g5, text="Dist. ao ponto (m):").grid(row=1, column=2, sticky="e", padx=6)
        ttk.Label(g5, textvariable=self.g_lbl_dist, foreground="#06c").grid(row=1, column=3, sticky="w")
    ...
    def capture_point(self):
        if not self.g_valid:
            ...
            return
        self.point_lat.set("{:.6f}".format(self.g_lat))
        self.point_lon.set("{:.6f}".format(self.g_lon))
        self._log(">> Ponto capturado em {:.6f}, {:.6f}.".format(self.g_lat, self.g_lon))
    ...
    def track_selected(self):
        ...
        if idx < 0 or idx >= len(self.tracks):
            return
        ponto = self.tracks[idx].get("ponto")
        if ponto and len(ponto) == 2:
            self.point_lat.set("{:.6f}".format(ponto[0]))
            self.point_lon.set("{:.6f}".format(ponto[1]))
            self._log(">> Ponto da pista carregado.")
        else:
            self._log(">> Esta pista ainda nao tem ponto salvo - capture no local (box).")

    def save_point_to_track(self):
        idx = self.track_cb.current()
        if idx < 0 or idx >= len(self.tracks):
            messagebox.showwarning("Pistas", "Selecione uma pista primeiro.")
            return
        ponto = self._point_ready()
        if not ponto:
            messagebox.showwarning("Ponto", "Capture ou preencha o ponto (lat, lon) antes de salvar.")
            return
        self.tracks[idx]["ponto"] = ponto
        try:
            with open(resource_path("tracks.json"), "w", encoding="utf-8") as fp:
                json.dump({"pistas": self.tracks}, fp, ensure_ascii=False, indent=2)
            self._log(">> Ponto salvo no banco para {}.".format(self.tracks[idx].get("nome", "?")))
        except Exception as e:
            messagebox.showerror("Pistas", "Nao consegui gravar tracks.json:\n{}".format(e))

    def _point_ready(self):
        try:
            return [float(self.point_lat.get().replace(",", ".")),
                    float(self.point_lon.get().replace(",", "."))]
        except ValueError:
            return None
    ...
    def send_gps(self):
        ...
        try:
            ...
            self.link.send("BDEB {}".format(int(self._f(self.bolim_debounce_s) * 1000)))
            ...
            self.link.send("GRANGE {}".format(int(self._f(self.range_m))))
            ponto = self._point_ready()
            if ponto:
                self.link.send("GPSPT {:.6f} {:.6f}".format(*ponto))
            self.link.send("SAVE")
    ...
    def _sim_render(self, lam, mavg, trend):
        ...
        low_th = self._sim_float(self.low_lambda, None)
        low = 1 if (self.low_enable.get() and low_th is not None and mavg <= low_th) else 0
        center = 1 if (not low and self.center_enable.get() and colmask == 0) else 0
        ...
        self.sim_lbl_alarm.set("SUPRIMIDO" if (alarm and low) else ("ATIVO" if alarm else "ok"))
        self._render_cells(self.sim_canvas, self.sim_cells, colmask, trend, int(alarm), center, low)
    ...
    def pick_low_color(self):
        c = colorchooser.askcolor(color=self.low_color, title="Cor do indicador abaixo do limite")
        if c and c[1]:
            self.low_color = c[1]
            self.low_btn.config(bg=c[1])
    ...
    def send_all_static(self):
        ...
        try:
            ...
            self.link.send("ABRIGHT {}".format(int(self.alert_bright.get())))
            ...
            lr, lg, lb = self.hex_to_rgb(self.low_color)
            self.link.send("LOW {} {} {} {} {}".format(self.low_enable.get(),
                                                       self._f(self.low_lambda), lr, lg, lb))
    ...
    def _handle_line(self, line):
        if line.startswith("D "):
            ...
            if len(p) >= 8:
                ...
                low = int(p[8]) if len(p) >= 9 else 0   # compat. firmware antigo
        ...
        if line.startswith("G "):
            ...
            if len(p) >= 10:
                try:
                    ...
                    self.g_lbl_fix.set("OK ({} sat)".format(sats) if self.g_valid
                                       else "procurando... ({} sat)".format(sats))
                    ...
                    if len(p) >= 11:
                        d = float(p[10])
                        self.g_lbl_dist.set("--" if d < 0 else "{:.0f}".format(d))
        ...
        for t in toks[1:]:
            ...
            try:
                if k.startswith("C") and k[1:].isdigit():
                    ...
                    en, lam, r, g, b = val.split(",")
                    self.low_enable.set(int(en))
                    self.low_lambda.set("{:.2f}".format(float(lam)))
                    self.low_color = "#{:02x}{:02x}{:02x}".format(int(r), int(g), int(b))
                    self.low_btn.config(bg=self.low_color)
                elif k == "MAP":
            ...
            try:
                if k == "EN":
                    ...
                    if abs(float(la)) > 1e-6 or abs(float(lo)) > 1e-6:
                        self.point_lat.set("{:.6f}".format(float(la)))
                        self.point_lon.set("{:.6f}".format(float(lo)))
                elif k == "RANGE":
                    self.range_m.set(val)
                elif k == "BOLIM":
                    lam, lim = val.split(",")
                    self.bolim_lambda.set(lam); self.bolim_limit.set(lim)
                elif k == "DEB":
                    self.bolim_debounce_s.set("{:g}".format(int(val) / 1000.0))
                elif k == "ROW":
                    r = int(val)
    ...
    def _render_cells(self, canvas, cells, colmask, trend, alarm, center, low=0):
        ...
        square_cells = {(3, 3), (3, 4), (4, 3), (4, 4)}
        # Indicador "abaixo do limite": mostra so o quadrado 2x2 central (cor low)
        # e substitui a barra. Tem prioridade sobre o alerta (nao pisca), igual
        # ao firmware -> painel "calmo" abaixo do limite.
        if low:
            for y in range(8):
                for x in range(8):
                    color = self.low_color if (x, y) in square_cells else "#1a1a1a"
        ...
        center_cells = square_cells if center else set()
        for y in range(8):
            for x in range(8):
    ...
    def _update_preview(self, colmask, trend, alarm, center, low=0):
        self._render_cells(self.canvas, self.cells, colmask, trend, alarm, center, low)
